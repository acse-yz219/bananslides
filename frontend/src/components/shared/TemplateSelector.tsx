...
export const TemplateSelector: React.FC<TemplateSelectorProps> = ({
    ...
    if (file) {
      try {
        if (showUpload) {
          // 主页模式：直接上传到用户模板库
          if (!currentUser) {
            show({ message: '请先登录以管理个人模板', type: 'info' });
            return;
          }
          ...
          if (saveToLibrary) {
            // 保存到模板库并应用
            if (!currentUser) {
              show({ message: '请先登录以保存到个人模板库', type: 'info' });
              // 未登录时仅应用到项目
              onSelect(file);
              return;
            }
  ...
  const handleDeleteUserTemplate = async (template: UserTemplate, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!currentUser) {
      show({ message: '请先登录以删除个人模板', type: 'info' });
      return;
    }
    ...
    <>
      <div className="space-y-4">
        ...
        {currentUser && userTemplates.length > 0 && (
          <div>
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">我的模板</h4>
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">预设模板</h4>
          <div className="grid grid-cols-4 gap-4">
            ...
            {presetTemplates.map((template) => (
              <div
                ...
                className={`aspect-[4/3] rounded-lg border-2 cursor-pointer transition-all bg-gray-100 dark:bg-gray-700 flex items-center justify-center relative ${
            ...
            {/* 上传新模板 */}
            <label className="aspect-[4/3] rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-purple-500 dark:hover:border-purple-500 cursor-pointer transition-all flex flex-col items-center justify-center gap-2 relative overflow-hidden bg-white dark:bg-gray-800">
              <span className="text-2xl text-gray-400 dark:text-gray-500">+</span>
              <span className="text-sm text-gray-500 dark:text-gray-400">上传模板</span>
              <input
                type="file"
                accept="image/*"
          ...
          {!showUpload && (
            <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              ...
              {!currentUser && (
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">未登录时仅应用到项目，无法保存到个人模板库</div>
              )}
            </div>
          )}
        </div>
        ...
        {projectId && (
          <div className="mt-4">
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">从素材库选择</h4>
            <Button
              variant="secondary"
              size="sm"
