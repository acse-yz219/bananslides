 ...
 * 批量生成图片
 */
export const generateImages = async (projectId: string, options?: { limit?: number }): Promise<ApiResponse> => {
  const response = await apiClient.post<ApiResponse>(
...
export const upgrade = async (plan: 'monthly' | 'annual'): Promise<ApiResponse<{ user: User }>> => {
  const response = await apiClient.post<ApiResponse<{ user: User }>>('/api/auth/upgrade', { plan });
  return response.data;
};
