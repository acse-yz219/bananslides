...
export const SlidePreview: React.FC = () => {
  ...
  const handleGenerateFirst5Pages = async () => {
    const hasImages = currentProject?.pages.slice(0, 5).some(
      (p) => p.generated_image_path
    );

    const executeGenerate = async () => {
      await generateImages({ limit: 5 });
    };
  ...
  const handleGenerateAll = async () => {
    // 检查Pro会员限制：非会员最多只能生成5页
    if (!currentUser?.is_pro && currentProject?.pages && currentProject.pages.length > 5) {
      confirm(
        '非会员最多只能生成5页PPT，是否去升级Pro会员？',
        () => navigate('/pricing'),
        { 
          title: '请升级Pro会员', 
          variant: 'info', 
          confirmText: '去升级',
          cancelText: '生成前五页',
          onCancel: handleGenerateFirst5Pages
        }
      );
      return;
    }
  ...
  const handleRegeneratePage = async () => {
    ...
    // 检查Pro会员限制：非会员只能生成前5页
    if (!currentUser?.is_pro && selectedIndex >= 5) {
      confirm(
        '非会员最多只能生成5页PPT，是否去升级Pro会员？',
        () => navigate('/pricing'),
        { title: '请升级Pro会员', variant: 'info', confirmText: '去升级' }
      );
      return;
    }
  ...
  const handleExport = async (type: 'pptx' | 'pdf') => {
    ...
    if (!currentUser?.is_pro) {
        show({ message: 'Export is a Pro feature. Please upgrade.', type: 'info' });
  ...
  return (
    <div className="h-screen bg-gray-50 dark:bg-gray-900 flex flex-col overflow-hidden">
      {/* 顶栏 */}
      <header className="h-14 md:h-16 bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-3 md:px-6 flex-shrink-0">
        <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
          <Button
            ...
            onClick={() => navigate('/')}
            className="hidden sm:inline-flex flex-shrink-0 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-white/10"
          >
          ...
          <Button
            ...
            className="flex-shrink-0 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-white/10"
          >
            <span className="hidden sm:inline">返回</span>
          </Button>
          <div className="flex items-center gap-1.5 md:gap-2 min-w-0">
            <img src="/logo.png" alt="Magic AiPPT" className="w-8 h-8 object-contain" />
            <span className="text-base md:text-xl font-bold truncate dark:text-white">Magic AiPPT</span>
          </div>
          <span className="text-gray-400 hidden md:inline">|</span>
          <span className="text-sm md:text-lg font-semibold truncate hidden sm:inline dark:text-gray-200">预览</span>
        </div>
        <div className="flex items-center gap-1 md:gap-3 flex-shrink-0">
          <Button
            ...
            className="hidden lg:inline-flex hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-white/10"
          >
          ...
          <Button
            ...
            className="hidden md:inline-flex hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-white/10"
          >
          ...
          <div className="relative">
            <Button
              ...
              className="text-xs md:text-sm bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-none shadow-lg shadow-purple-500/20"
            >
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-w-0 min-h-0">
        {/* 左侧：缩略图列表 */}
        <aside className="w-full md:w-80 bg-white dark:bg-gray-800 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 flex flex-col flex-shrink-0">
          <div className="p-3 md:p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0 space-y-2 md:space-y-3">
            <Button
              variant="primary"
              icon={<Sparkles size={16} className="md:w-[18px] md:h-[18px]" />}
              onClick={handleGenerateAll}
              className="w-full text-sm md:text-base bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-none shadow-lg shadow-purple-500/20"
            >
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-2 md:pt-3">
              <button
                ...
                className="w-full flex items-center justify-between text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 transition-colors"
              >
                <span>额外要求</span>
                ...
                <div key={page.id} className="md:w-full flex-shrink-0">
                  ...
                  >
                    {page.generated_image_path ? (
                      ...
                      <div className="w-full h-full bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center text-xs text-gray-400">
                        {index + 1}
                      </div>
                    )}
                  </button>
        ...
        {/* 右侧：大图预览 */}
        <main className="flex-1 flex flex-col bg-gradient-to-br from-purple-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 min-w-0 overflow-hidden">
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 flex items-center justify-center p-4 md:p-8">
                <div className="max-w-5xl w-full">
                  <div className="relative aspect-video bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden touch-manipulation">
                          ...
                          {(!selectedPage?.id || !pageGeneratingTasks[selectedPage.id]) && 
                           selectedPage?.status !== 'GENERATING' && (
                            <Button
                              ...
                              className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-none shadow-lg shadow-purple-500/20"
                            >
                              生成此页
                            </Button>
                          )}
              ...
              {/* 控制栏 */}
              <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-3 md:px-6 py-3 md:py-4 flex-shrink-0">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 max-w-5xl mx-auto">
                  ...
                  <div className="flex items-center gap-1.5 md:gap-2 w-full sm:w-auto justify-center">
                            ...
                            {imageVersions.map((version) => (
                              <button
                                ...
                                className={`w-full px-3 md:px-4 py-2 text-left hover:bg-gray-50 transition-colors flex items-center justify-between text-xs md:text-sm ${
                                  version.is_current ? 'bg-purple-50' : ''
                                }`}
                              >
                    ...
                    <Button
                      ...
                      className="text-xs md:text-sm flex-1 sm:flex-initial hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-white/10"
                    >
                      编辑
                    </Button>
                    <Button
      ...
      >
        <div className="space-y-4">
          {/* 图片（支持矩形区域选择） */}
          <div
            className="aspect-video bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden relative"
            ...
            {imageUrl && (
              <>
                ...
                <button
                  ...
                  className="absolute top-2 left-2 z-10 px-2 py-1 rounded bg-white/80 dark:bg-gray-800/80 text-[10px] text-gray-700 dark:text-gray-300 hover:bg-purple-50 dark:hover:bg-gray-700 shadow-sm flex items-center gap-1"
                >
                  <Sparkles size={12} />
                ...
                {selectionRect && (
                  <div
                    className="absolute border-2 border-purple-500 bg-purple-400/10 pointer-events-none"
                    style={{
          ...
          {selectedPage?.outline_content && (
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              >
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">页面大纲</h4>
                {isOutlineExpanded ? (
          ...
          {selectedPage?.description_content && (
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              ...
              >
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">页面描述</h4>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 space-y-4">
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">选择上下文图片（可选）</h4>
            ...
            {currentProject?.template_image_path && (
              <div className="flex items-center gap-3">
                ...
                <label htmlFor="use-template" className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700 dark:text-gray-300">使用模板图片</span>
                  {currentProject.template_image_path && (
                    <img
                      src={getImageUrl(currentProject.template_image_path, currentProject.updated_at)}
                      alt="Template"
                      className="w-16 h-10 object-cover rounded border border-gray-300 dark:border-gray-600"
                    />
                  )}
                </label>
              </div>
            ...
            {selectedPage?.description_content && (() => {
              ...
              return descImageUrls.length > 0 ? (
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">描述中的图片：</label>
                    ...
                    {descImageUrls.map((url, idx) => (
                      <div key={idx} className="relative group">
                        <img
                          ...
                          className="w-full h-20 object-cover rounded border-2 border-gray-300 dark:border-gray-600 cursor-pointer transition-all"
                          style={{
                            borderColor: selectedContextImages.descImageUrls.includes(url)
                              ? '#9333ea'
                              : undefined, // fallback to class border
                          }}
                          onClick={() => {
                            setSelectedContextImages((prev) => {
                        ...
                        {selectedContextImages.descImageUrls.includes(url) && (
                          <div className="absolute inset-0 bg-purple-500/20 border-2 border-purple-500 rounded flex items-center justify-center">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <label className="w-20 h-20 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 transition-colors">
