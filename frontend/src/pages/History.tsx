...
export const History: React.FC = () => {
  ...
  const handleSelectProject = useCallback(async (project: Project) => {
    ...
    try {
      ...
      if (storageKey) localStorage.setItem(storageKey, projectId);
  ...
  }, [selectedProjects, editingProjectId, setCurrentProject, syncProject, navigate, getProjectRoute, show, currentUser]);
  ...
  // ===== 删除操作 =====
    ...
    const deleteProjects = useCallback(async (projectIds: string[]) => {
      setIsDeleting(true);
    const storageKey = currentUser?.user_id ? `currentProjectId:${currentUser.user_id}` : null;
    const currentProjectId = storageKey ? localStorage.getItem(storageKey) : null;
  ...
  }, [setCurrentProject, show, currentUser]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      {/* 导航栏 */}
      <nav className="h-14 md:h-16 bg-white dark:bg-gray-800 shadow-sm border-b border-gray-100 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-3 md:px-4 h-full flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Magic AiPPT</span>
          </div>
          <div className="flex items-center gap-2 md:gap-4">
            <Button
              ...
              onClick={() => navigate('/')}
              className="text-xs md:text-sm hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-300"
            >
      ...
      <main className="max-w-6xl mx-auto px-3 md:px-4 py-6 md:py-8">
        <div className="mb-6 md:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1 md:mb-2">历史项目</h1>
