...
import { ArrowLeft, Save, ArrowRight, Plus, FileText, Sparkle, Sparkles } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
export const OutlineEditor: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
      {/* 顶栏 */}
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 px-3 md:px-6 py-2 md:py-3 flex-shrink-0">
        <div className="flex items-center justify-between gap-2 md:gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <img src="/logo.png" alt="Magic AiPPT" className="w-8 h-8 md:w-8 md:h-8 object-contain" />
              <span className="text-base md:text-xl font-bold dark:text-white">Magic AiPPT</span>
            </div>
            <span className="text-gray-400 dark:text-gray-500 hidden lg:inline">|</span>
            <span className="text-sm md:text-lg font-semibold hidden lg:inline dark:text-gray-200">编辑大纲</span>
          </div>
      ...
      {/* 上下文栏 */}
      <div className="bg-purple-50 dark:bg-gray-800 border-b border-purple-100 dark:border-gray-700 px-3 md:px-6 py-2 md:py-3 max-h-32 overflow-y-auto flex-shrink-0">
        <div className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
          {currentProject.creation_type === 'idea' && (
            <span className="font-medium text-gray-700 dark:text-gray-300 flex-shrink-0 flex items-center">
              <Sparkle size={12} className="mr-1" /> PPT构想:
              <span className="text-gray-900 dark:text-gray-100 font-normal ml-2 break-words whitespace-pre-wrap">{currentProject.idea_prompt}</span>
            </span>
          )}
          {currentProject.creation_type === 'outline' && (
            <span className="font-medium text-gray-700 dark:text-gray-300 flex-shrink-0 flex items-center">
              <FileText size={12} className="mr-1" /> 大纲:
              <span className="text-gray-900 dark:text-gray-100 font-normal ml-2 break-words whitespace-pre-wrap">{currentProject.outline_text || currentProject.idea_prompt}</span>
            </span>
          )}
          {currentProject.creation_type === 'descriptions' && (
            <span className="font-medium text-gray-700 dark:text-gray-300 flex-shrink-0 flex items-center">
              <FileText size={12} className="mr-1" /> 描述:
              <span className="text-gray-900 dark:text-gray-100 font-normal ml-2 break-words whitespace-pre-wrap">{currentProject.description_text || currentProject.idea_prompt}</span>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4 md:mb-6">
              <Button
                ...
                onClick={addNewPage}
                className="w-full sm:w-auto text-sm md:text-base bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-none shadow-lg shadow-purple-500/20"
              >
                添加页面
              </Button>
              {currentProject.pages.length === 0 ? (
                <Button
                  ...
                  className="w-full sm:w-auto text-sm md:text-base hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 dark:hover:text-purple-400"
                >
        ...
        {/* 右侧：预览 */}
        <div className="hidden md:block w-96 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 p-4 md:p-6 overflow-y-auto flex-shrink-0">
          <h3 className="text-base md:text-lg font-semibold text-gray-900 dark:text-white mb-3 md:mb-4">预览</h3>
          
          {selectedPage ? (
            <div className="space-y-3 md:space-y-4">
              <div>
                <div className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-1">标题</div>
                <div className="text-base md:text-lg font-semibold text-gray-900 dark:text-white">
                  {selectedPage.outline_content.title}
                </div>
              </div>
              <div>
                <div className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-2">要点</div>
                <ul className="space-y-1.5 md:space-y-2">
                  {selectedPage.outline_content.points.map((point, idx) => (
                    <li key={idx} className="flex items-start text-sm md:text-base text-gray-700 dark:text-gray-300">
            ...
            <div className="text-center py-8 md:py-10 text-gray-400 dark:text-gray-500">
        ...
        {/* 移动端预览：底部抽屉 */}
        {selectedPage && (
          <div className="md:hidden fixed inset-x-0 bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 max-h-[50vh] overflow-y-auto shadow-lg z-50">
            ...
            <div className="space-y-2">
              ...
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">要点</div>
                <ul className="space-y-1">
                  {selectedPage.outline_content.points.map((point, idx) => (
                    <li key={idx} className="flex items-start text-xs text-gray-700 dark:text-gray-300">
                      <span className="mr-1.5 text-purple-500 flex-shrink-0">•</span>
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
