...
interface ProjectState {
  ...
  generateImages: (options?: { limit?: number }) => Promise<void>;
...
export const useProjectStore = create<ProjectState>((set, get) => {
  const getProjectStorageKey = () => {
    const uid = useAuthStore.getState().currentUser?.user_id;
    return uid ? `currentProjectId:${uid}` : null;
  };
...
const debouncedUpdatePage = debounce(
  ...
  initializeProject: async (type, content, templateImage) => {
    ...
    try {
      ...
      if (project) {
        ...
        if (key) localStorage.setItem(key, project.id!);
      }
    } catch (error: any) {
  ...
  syncProject: async (projectId?: string) => {
    ...
    if (!targetProjectId) {
      if (currentProject?.id) {
        ...
        const key = getProjectStorageKey();
        targetProjectId = key ? localStorage.getItem(key) || undefined : undefined;
      }
    }
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 403) {
          ...
          set({ error: normalizeErrorMessage('无权限访问该项目，已清除本地记录') });
          return;
        }
  ...
  // 生成图片
  generateImages: async (options?: { limit?: number }) => {
    const { currentProject, startAsyncTask } = get();
    if (!currentProject) return;

    await startAsyncTask(() => api.generateImages(currentProject.id, options));
  },
