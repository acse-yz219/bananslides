...
def _check_page_access(project_id: str, page_id: str = None):
    """Check if current user has access to the page/project"""
    project = Project.query.get(project_id)
    
    if not project:
        return None, None, not_found('Project')
    
    if project.user_id != g.current_user.id:
        return None, None, error_response('FORBIDDEN', 'You do not have access to this project', 403)
    
    if page_id:
        page = Page.query.get(page_id)
        if not page or page.project_id != project_id:
            return None, None, not_found('Page')
...
def create_page(project_id):
    ...
    """
    try:
        project, _, error = _check_page_access(project_id)
        if error:
            return error
