...
def list_projects():
    ...
    try:
        ...
        projects = Project.query.filter_by(user_id=user_id).order_by(desc(Project.updated_at)).limit(limit).offset(offset).all()
...
def create_project():
    ...
    try:
        ...
        if not g.current_user.is_pro:
            project_count = Project.query.filter_by(user_id=g.current_user.id).count()
            if project_count >= 3:
                return error_response('FORBIDDEN', 'Free users can only create up to 3 projects. Please upgrade to Pro.', 403)
...
def generate_from_description(project_id):
    ...
    """
    try:
        project, error = _check_project_access(project_id)
        if error:
            return error
...
def generate_images(project_id):
    ...
    try:
        ...
        task.set_progress({
            'total': limit if limit else len(pages),
            'completed': 0,
            'failed': 0
        })
        ...
        return success_response({
            ...
            'total_pages': limit if limit else len(pages)
        }, status_code=202)
